...
'''
widget_text_input = pn.widgets.AutocompleteInput(
    name='Your process', options=list_product,
    case_sensitive=False, search_strategy='includes',
    placeholder='Write something here')
print(widget_text_input.value)
...
def find_database_activity(name):
    # https://docs.brightway.dev/en/latest/content/gettingstarted/objects.html#object-selection
    selected_activity = [
        node for node in useeio
        if name == node['name']
...
def update_interactive_elements(event):
    ...
    widget_static_text.value = type(widget_text_input.value)
    selected_activity = find_database_activity(widget_text_input.value)
    
    #widget_static_text.value = selected_activity
...
# %%
useeio.search('electricity')
# %%
name_test=[act for act in useeio if "gas" in act['name'].lower() and 'product' in act['type']]
name_test
